...
#         plt.scatter(train_set[:,0],train_set[:,1],c=new_y,label=str(C_))
#         plt.show()
#         plt.legend()
        ...
        plt.plot(xrange_pts,y_vals,label="C : "+str(C_),alpha=0.3)
...
#     train_data[:, 0],train_data[:, 1] = train_data[:, 1], train_data[:, 0].copy()
    ...
    train_data[:,0:2] = train_data[:,0:2]**2
    ...
    for dat_ind in range(0,len(test_label)):
        if test_label[dat_ind] == -1:
            color_label_test.append('red')
        elif test_label[dat_ind] == 1:
            color_label_test.append('blue')    
    
    
    plt.figure()
    plt.scatter(test_X[:,0],test_X[:,1],c = color_label_test)
#     plt.scatter(train_data[:,0],train_data[:,1],c = color_label)
    box_leg = []
...
#     plt.show()
    ...
    train_data = np.r_[train_data[:,0:2],test_X]
